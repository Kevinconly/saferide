...
export default function RidesPage() {
  const { data, isLoading } = useQuery({
    queryKey: ["rides"],
    queryFn: async () =>
      api.get<{ items: Ride[]; total: number }>("/rides?pageSize=50"),
  });
  ...
  return (
    <div>
      <PageHeader
        title="Ride history"
        subtitle={data ? `${data.total} ride(s)` : undefined}
      />
      {isLoading ? (
        ...
        <Card>
          <EmptyState
            title="No rides yet"
            hint="Request your first ride from the book page."
          />
        </Card>
      ) : (
